...
import BahiGoCartItemComponent from '../components/BahiGoCartItemComponent';
...
export default function () {
  ...
  return (
    <View style={styles.container}>
      ...
      {!cart.length && (
        <>
          <Text style={styles.text}>{'ПУСТО!'.toUpperCase()}</Text>
        </>
      )}
        ...
        <>
          <View style={{height: height * 0.7}}>
            <ScrollView style={styles.flex} contentContainerStyle={styles.main}>
              {cart.map((item, index) => (
                <BahiGoCartItemComponent item={item} key={index} />
              ))}
            </ScrollView>
          </View>
        </>
